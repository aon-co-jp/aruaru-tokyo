...
- [poem-cosmo-tauri](https://github.com/aon-co-jp/poem-cosmo-tauri) — Poem/Tauri実装規約の出典元、このリポジトリの技術スタック規約はここに準拠
...
- audiocafe.tokyo(PHP、別リポジトリ管理外・VPS上の運用ディレクトリ) — `/aruaru/`・`/aruaru-lady/`・`/rakuten-mobile/`ミラーlocationの実体
